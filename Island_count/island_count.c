...
struct node** read_graph() {
    ...
    // Pointer to an array of pointers which will
    ...
    // For loop used to connect all the links
    // to the particular node using linked list
    ...
    // Logic to free the Dynamic memory which is
    // allocated via linked list
...
struct node** insert_node(int vi, int vj, struct node** head) {
    ...
    // If in the array of Heads that location is null simply
    // store the address of new node in it
    if (*(head + vi) == NULL) {
        *(head + vi) = q;
    }

    // If it is not null then store the head in some
    // temporary variable then goto the end
