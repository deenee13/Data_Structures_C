...
struct bstnode* delete_node(struct bstnode* root, int data)
{
    ...
    {   
        ...
        {
            struct bstnode* temp = root; // Storing the address of the parent into the temp node

            // Updating the address of the parent node with the the address of the right node as to delete it 
            root = root->right;
 ...
 * to two functions. Which was very expensive. Thus over here we are seeting the upper and lower 
...
bool is_subtree_greater(struct bstnode* root, int data)
{
    ...
    if(root->data > data && is_subtree_greater(root->left, data) && is_subtree_greater(root->right, data))
    {
        return true;
    }
    else
...
bool is_subtree_lesser(struct bstnode* root, int data)
{
    ...
    if(root->data <= data && is_subtree_lesser(root->left, data) && is_subtree_lesser(root->right, data))
    {
        return true;
    }
    else
...
bool is_binary_search_tree(struct bstnode* root)
{
    ...
    if( is_subtree_lesser(root->left, root->data) && is_subtree_greater(root->right, root->data) && 
    ...
    // return(is_bst_util(root, INT_MIN, INT_MAX)); /* Different approach for same problem uncomment to try*/
}
